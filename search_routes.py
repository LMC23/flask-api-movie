...
def search():
    ...
    try:
        ...
        # url for data source
        url_search = (
            "https://www.cinemacity.ro/ro/data-api-service/v1/quickbook/10107/films/until/2023-02-28?attr=&lang=ro_RO"
        )
        ...
        for film in films_search:
            if name.lower() in film["name"].lower():
                ...
                film_id = film.get("id")
                url_theater = f"https://www.cinemacity.ro/ro/data-api-service/v1/quickbook/10107/cinema-events/in-group/bucuresti/with-film/{film_id}/at-date/{date}?attr=&lang=ro_RO"  # noqa: E501
                r_theater = requests.get(url_theater)
                response_theater = r_theater.json()

                # stores data regarding cinemas and events
                cinemas = response_theater.get("body").get("cinemas")
                events = response_theater.get("body").get("events")
                cinema_info = []
                ...
                for cinema in cinemas:
                    ...
                    # add a dictionary to event_info list when the event
                    # corresponds with the current cinema and searched movie
                    for event in events:
